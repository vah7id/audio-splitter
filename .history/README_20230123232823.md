...
8. Start backend in separate terminal
    ```sh
    (env) spleeter-web$ python manage.py collectstatic && python manage.py runserver 127.0.0.1:8080
    ````
...
10. Launch **Spleeter Web**
    ...
    Navigate to [http://127.0.0.1:8080](http://127.0.0.1:8080) in your browser. Uploaded and mixed tracks will appear in `media/uploads` and `media/separate` respectively.
...
4. Access **Spleeter Web** at whatever you set `APP_HOST` to. Note that it will be running on port 80, not 8080. You can change this by setting `NGINX_PORT` and `NGINX_PORT_SSL`.
