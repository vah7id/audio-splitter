...
class SongTable extends React.Component<Props> {
  ...
  componentDidUpdate(prevProps, prevState) {
    const fntmp = async() => {
      ...
      if( (prevProps.data && prevProps.data.length === 0 &&  this.props.data.length > 0 &&  this.props.data[0].id) || (prevProps.data && prevProps.data[0].id && this.props.data[0].id && prevProps.data[0].id !== this.props.data[0].id) ) {
        if(el) {
          el.forEach((ch, index) => {
            if(ch) {
              const loadedWaves = document.querySelectorAll('.waveSurfer-el wave wave');
