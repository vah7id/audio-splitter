...
const modelFormatter: ColumnFormatter<MixItem> = (cellContent, row) => {
  ...
  const badge = (
    <Badge variant="dark" style={shouldShowTooltip ? { cursor: 'pointer' } : {}}>
      {separatorLabelMap[separator]} Algorithm
    </Badge>
  );
  ...
  render(): JSX.Element {
    ...
    const columns: ColumnDescription[] = [
      ...
      },*/
      {
        ...
        style: () => {
          return { maxWidth: '150px', paddingRight: 28 };
        },
      },
    ];
    ...
    if (data.length > 0) {
      ...
      return (
        <div className="m-4 text-center">
          <p>No mixes. Click &ldquo;Seperate Layouts&rdquo; or &ldquo;Vocal Remover&rdquo; to create one.</p>
        </div>
      );
    }
  }
