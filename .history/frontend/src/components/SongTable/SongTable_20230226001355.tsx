...
const loadWaveSurfer: ColumnFormatter<SongData> = (cell,row) => {
  return (
        <div className="waveSurfer-el" id={`waveform-${row.id}`} style={{width: '100%', margin: '-32px 0 0 0'}} />
  );
}
...
class SongTable extends React.Component<Props> {
  ...
  componentDidUpdate(prevProps, prevState) {
    const fntmp = async() => {
      ...
      if(prevProps.data && prevProps.data.length === 0 &&  this.props.data.length > 0 &&  this.props.data[0].id)

      if(prevProps.data && prevProps.data[0].id && this.props.data[0].id && prevProps.data[0].id !== this.props.data[0].id) {
        if(el) {
          el.forEach((ch, index) => {
            if(ch) {
