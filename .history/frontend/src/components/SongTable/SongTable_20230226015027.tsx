...
class SongTable extends React.Component<Props> {
  ...
  componentDidUpdate(prevProps, prevState) {
    const fntmp = async() => {
      ...
      if( (prevProps.data && prevProps.data.length === 0 &&  this.props.data.length > 0 &&  this.props.data && this.props.data[0].id) ||
       (prevProps.data && prevProps.data[0].id && this.props.data && this.props.data[0] && prevProps.data[0].url !== this.props.data[0].url) ) {
        if(el) {
          el.forEach((ch, index) => {
            if(ch) {
              const loadedWaves = document.querySelectorAll('.waveSurfer-el wave wave');
