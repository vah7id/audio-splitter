...
const loadWaveSurfer: ColumnFormatter<SongData> = (cell,row) => {
  return (
        <div className="waveSurfer-el" id={`waveform-${row.id}`} style={{width: '100%', margin: '-16px 0 0 0'}} />
  );
}
  ...
  return (
    <div className="d-flex align-items-center justify-content-end">
      <TextButton  className="pl-1 btn-sm" variant="warning" disabled={disabled} onClick={onDynamicMixClick} song={row}>
        ...
        <span className="align-middle"> Split Audio Layouts</span>
      </TextButton>
      <TextButton className="pl-1 btn-sm" variant="secondary" disabled={disabled} onClick={onStaticMixClick} song={row}>
...
class SongTable extends React.Component<Props> {
  componentDidMount() {
    ...
    setTimeout(async() => {
      ...
      const loadedWaves = document.querySelectorAll('.waveSurfer-el wave wave');
      if(el) {
        el.forEach((ch, index) => {
          if(ch) {
            ch.click();
            if(loadedWaves.length < 1) {
              ...
              wavesurferInstance.load('https://mikrotakt.app/'+this.props.data[index].url)
            }
          }
        })
      }
  ...
  componentDidUpdate(prevProps, prevState) {
    const fntmp = async() => {
      ...
      if((prevProps.data && prevProps.data.length === 0 && this.props.data && this.props.data.length > 0 && this.props.data[0].url && this.props.data[0].url.indexOf('media/uploads') > -1) || 
        (prevProps.data && prevProps.data.length === 0 &&  this.props.data.length > 0 &&  this.props.data[0].url && this.props.data[0].url.indexOf('media/uploads') > -1) ||
       (prevProps.data.length > 0 && prevProps.data[0].url !== "" && this.props.data.length > 0  && prevProps.data[0].url !== this.props.data[0].url) ) {
        if(el) {
          el.forEach((ch, index) => {
            if(ch) {
              //ch.click();
              console.log('***********************');
              console.log(document.getElementById('waveform-'+this.props.data[index].id).length)
              console.log(document.getElementById('waveform-'+this.props.data[index].id))
              console.log('###################')
              const loadedWaves = document.querySelectorAll('.waveSurfer-el wave wave');
              const el2 = document.getElementById('waveform-'+this.props.data[index].id);
              if(el2 && loadedWaves.length < 1) {
      ...
      if(el && this.props.data && this.props.data[0].url && this.props.data[0].url.indexOf('media/uploads') > -1) {
        el.forEach((ch, index) => {
          if(ch) {
           // ch.click();
            const loadedWaves = document.querySelectorAll('.waveSurfer-el wave wave');
  ...
  render(): JSX.Element {
    ...
    const columns: ColumnDescription<SongData>[] = [
      ...
      },*/
      {
        ...
        style: () => {
          return { width: '40px', fontSize: '20px' };
        },
      },
      {
        dataField: 'wave',
