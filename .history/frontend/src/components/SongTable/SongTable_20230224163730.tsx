...
const spleetColFormatter: ColumnFormatter<SongData> = (cell, row, rowIndex, formatExtraData) => {
  ...
  return (
    <div className="d-flex align-items-center justify-content-end">
      <TextButton  className="pl-1" variant="warning" disabled={disabled} onClick={onDynamicMixClick} song={row}>
         <Layers style={{marginLeft: '12px !important'}} className="align-middle" size={16} />
        <span className="align-middle"> Seperate Layouts</span>
      </TextButton>
      <TextButton className="pl-1" variant="dark" disabled={disabled} onClick={onStaticMixClick} song={row}>
         <Scissors style={{marginLeft: '12px !important'}} className="align-middle" size={16} />
...
class SongTable extends React.Component<Props> {
  componentDidMount() {
    setTimeout(() => {
      const el = document.querySelector('.expand-cell');
      if(el) {
        document.querySelector('.expand-cell').click();
      }
    }, 2000)
  }
  render(): JSX.Element {
