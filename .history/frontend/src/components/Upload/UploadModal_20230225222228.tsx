...
class UploadModal extends React.Component<Props, State> {
  ...
  onNext = (): void => {
    ...
    const itemTable = document.querySelectorAll('.react-bootstrap-table');

    if(itemTable.length === 1 || itemTable.length === 2) {
      this.setState({
    ...
    return (
        ...
        >
          ...
          {detailsStep ? (
            ...
            <>
              <Dropzone
                disabled={!!link || this.state.disabledLimit}
                maxFiles={1}
                multiple={false}
              ...
              <YouTubeForm
                ...
                disabled={droppedFile || this.state.disabledLimit}
                value={query || link}
